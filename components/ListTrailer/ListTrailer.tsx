...
function ListTrailer() {
    ...
    const renderTrailerItem = () => {
        ...
        return trailerItems[typeOfTrailer].map((item: TrailerItems) => <TrailerItem poster_path={item.poster_path} name={item.name} video_name={item.video_name} path={item.path} id={item.id} />)
    }
